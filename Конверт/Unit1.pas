...
begin
  ...
  c:=StrToInt(Edit3.Text);
  d:=StrToInt(Edit4.Text);
  if (a>c) and (b>d) then Label7.Caption:='Можно вложить второй конверт в первую.';
  if (c>a) and (d>b) then Label7.Caption:='Можно вложить первый конверт во вторую.';
  if (a>=c) and (b<=d) then Label7.Caption:='Нельзя вложить один конверт в другую.';
  if (a<=c) and (b>=d) then Label7.Caption:='Нельзя вложить один конверт в другую.';
end;
